...
| Feature | Description |
|---------|-------------|
| 🖥️ **Visual Wizard** | .NET WinForms GUI — install, launch, manage models visually |
| 🔌 **Plug & Play** | Insert USB, double-click `QPawWizard.exe` to start |
...
### Option A: Visual Wizard (Recommended for Windows)

1. Copy the entire `Q-Paw` folder to your USB drive
2. Double-click **`QPawWizard.exe`** — a self-contained .NET 8 app, no .NET runtime needed
3. Follow the visual wizard:
   - **📥 安装向导** — One-click install uv + Python + QwenPaw
   - **🔌 API 配置** — Configure DashScope / OpenAI / OpenRouter / ModelScope API keys
   - **🚀 启动助手** — Launch QwenPaw in online or offline mode
   - **📦 模型管理** — Download / import / delete models
   - **⚙️ 设置** — Package manager, mirrors, mode switching
   - **🔄 数据迁移** — Merge data from old Q-Paw versions
   - **💻 控制台** — Real-time command output
...
```
Q-Paw/                          ← USB Root
├── QPawWizard.exe              # 🖥️ Visual Wizard (self-contained .NET 8)
├── index.html                  # Web Console (open in browser)
│
...
│   ├── api-keys.env            # API keys (generated by Wizard)
│   ├── qwenpaw-config.yaml    # QwenPaw config (generated by Wizard)
...
## QPawWizard.exe — Visual Wizard

The `QPawWizard.exe` is a self-contained .NET 8 WinForms application that provides a visual interface for all Q-Paw operations. It requires **no .NET runtime installation** on the target PC.

### Features

| Page | Function |
|------|----------|
| 🏠 **仪表盘** | System status overview — Python, QwenPaw, models, package manager |
| 📥 **安装向导** | One-click install: uv → Python → QwenPaw → modelscope → config |
| 🚀 **启动助手** | Launch QwenPaw in online/offline mode with proper env vars |
| 📦 **模型管理** | Download from ModelScope, import local models, delete models |
| 🔄 **更新 QwenPaw** | One-click update uv + QwenPaw + modelscope (also via `update.bat` / `update.sh`) |
| 🔌 **API 配置** | DashScope / OpenAI / OpenRouter / ModelScope API key management |
| ⚙️ **设置** | Package manager, download mirror, model mode, cache cleanup |
| 🔄 **数据迁移** | Merge models + data + config from old Q-Paw directory |
| 💻 **控制台** | Real-time command execution output |

### Technical Details

- **Framework**: .NET 8 WinForms (C#)
- **Build**: Self-contained single-file, `win-x64`, compressed
- **Size**: ~69MB (includes .NET runtime, no separate install needed)
- **Source**: See `Q-Paw-App/QPawWizard/` directory for full source code
- **Rebuild**: Run `build.bat` (Windows) or `build.sh` (Linux/macOS cross-compile)
...
2. Configure via QPawWizard.exe → 🔌 API 配置, or edit `config/portable.env`:
   ```
...
Run setup first (via QPawWizard.exe or setup.bat) to install Python and dependencies. After that, launch anytime.

### Q: Do I have to download models?
No! Online mode works without local models. You just need network + API Key.

### Q: QPawWizard.exe is 69MB, do I need .NET?
No! It's a self-contained single-file executable. Just double-click to run, no .NET installation needed.
...
Use QPawWizard.exe → 🔄 数据迁移, or run `migrate.bat` / `migrate.sh`.

### Q: How to update QwenPaw to the latest version?
Three ways to update:
1. **QPawWizard.exe** → Click "🔄 更新 QwenPaw" button on Dashboard or Launch page
...
| GUI Wizard | None | .NET WinForms visual wizard |
